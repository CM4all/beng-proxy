...
strref_set_dup_impl(pool_t pool, struct strref *dest, const struct strref *src
                    TRACE_ARGS_DECL)
{
    assert(dest != NULL);
    assert(src != NULL);
...
strref_append_impl(pool_t pool, struct strref *dest, const struct strref *src
                   TRACE_ARGS_DECL)
{
    char *p;
...
static __attr_always_inline char *
strref_dup_impl(pool_t pool, const struct strref *s
                TRACE_ARGS_DECL)
{
    assert(pool != NULL);
    assert(s != NULL);
