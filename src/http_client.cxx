...
struct HttpClient final : IstreamHandler {
    ...
    bool CheckDirect() const {
        ...
        assert(response.read_state == Response::State::BODY);
...
{
    assert(response.read_state == Response::State::STATUS ||
           response.read_state == Response::State::HEADERS);
...
HttpClient::AbortResponse(GError *error)
{
    assert(response.read_state == Response::State::STATUS ||
           response.read_state == Response::State::HEADERS ||
           response.read_state == Response::State::BODY);

    if (response.read_state != Response::State::BODY)
...
HttpClient::HandleLine(const char *line, size_t length)
{
    ...
    if (response.read_state == Response::State::STATUS)
...
{
    assert(client->response.read_state == HttpClient::Response::State::BODY);
...
HttpClient::ParseHeaders(const void *_data, size_t length)
{
    ...
    while ((end = (const char *)memchr(start, '\n',
        ...
        if (response.read_state != Response::State::HEADERS) {
...
HttpClient::FeedHeaders(const void *data, size_t length)
{
    ...
    if (response.status == HTTP_STATUS_CONTINUE) {
        ...
        if (request.body == nullptr) {
            ...
            response.read_state = Response::State::STATUS;
#endif
            AbortResponseHeaders(error);
            return BufferedResult::CLOSED;
        }
...
#ifndef NDEBUG
            /* assertion workaround */
            response.read_state = HttpClient::Response::State::STATUS;
#endif
            AbortResponseHeaders(error);
            return BufferedResult::CLOSED;
        }
...
http_client_socket_remaining(size_t remaining, void *ctx)
{
    ...
    if (client->response.read_state < HttpClient::Response::State::BODY)
...
HttpClient::OnError(GError *error)
{
    ...
    if (response.read_state != HttpClient::Response::State::BODY)
        AbortResponseHeaders(error);
