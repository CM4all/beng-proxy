...
is followed by the packets \texttt{URI} and \texttt{STATUS}.  The
translation response contains the location of another resource.  If
the translation response is empty, or if the error document itself
fails, then \emph{beng-proxy} generates a the default error page.  The
\texttt{ERROR\_DOCUMENT} translation request is only sent when the
resource's translation response contained an empty
\texttt{ERROR\_DOCUMENT} packet, indicating that the translation
server is willing to provide a custom error document. \\
...
\texttt{PAIR} & adds a FastCGI parameter in the form
\texttt{KEY=VALUE} \\
