...
\verb|SUBST_YAML_FILE| & Substitute variables in the form
"\verb|{[NAME]}|" with values from the given YAML file.  The values
are inserted raw into the stream, i.e. without any escaping/quoting.
The payload consists of 3 strings separated by a null byte: the prefix
for variable names (example: ``\verb|foo.|''; may be empty; will be
stripped before looking up in the YAML file), the YAML file path
...
be empty).  Example: ``\verb|foo.\0/the/file.yaml\0inside.file|''.
Inside the specified YAML map, child maps can be addressed using the
dot as a separator, e.g. ``\verb|foo.bar|'' looks up the key
\verb|bar| in a map found under key \verb|foo|. \\
