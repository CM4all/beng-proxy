...
\texttt{UID} & Follows \texttt{PIPE}, \texttt{CGI} or
\texttt{FASTCGI}, and sets the user id for the spawned process.  This
is a \texttt{uid\_t}, optionally followed by one or more
\texttt{gid\_t}.  If unset, then \texttt{beng-proxy} will run the sub
process as \texttt{nobody:nogroup}.  Future versions may refuse to
spawn processes without this packet. \\
