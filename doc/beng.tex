...
it may be referenced later; \texttt{iframe} puts the widget into an
...
allow this, \emph{beng-proxy} can rewrite relative links in the
following modes:
...
\texttt{focus} & links are also relative to the widget, but the
widget stays embedded in the template \\

\hline

\texttt{proxy} & links are also relative to the widget, and the target
is proxied by \emph{beng-proxy} (useful for images or other non-HTML
resources) \\
...
The mode name must be specified in the element attribute
\texttt{c:base} before the attribute containing the URI.  Example:

\begin{verbatim*}
<img c:base="proxy" href="foo.jpg"/>
\end{verbatim*}
...
[null] & the full page \\
\hline
\texttt{frame} & just this one widget, processor enabled (must be
\texttt{text/html}) \\
\hline
\texttt{proxy} & this widget, but processor enabled, any content type
allowed \\
\hline
\end{tabular}
