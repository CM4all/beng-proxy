...
class Widget {
  ...
  string translateURI(string uri, string mode);
  XMLHTTPRequest get(string uri, function onreadystatechange);
  XMLHTTPRequest post(string uri, string content_type, request_body,
...
server URI to an absolute URL to \emph{beng-proxy}.  The following
modes are available:
...
The methods \texttt{get} and \texttt{post} use the \texttt{proxy}
mode; \texttt{reload} sets the \texttt{frame} mode.
