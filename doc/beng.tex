...
\verb|allow_user| & Allow child processes to impersonate the
given user. \\\hline

\verb|allow_group| & Allow child processes to impersonate the given
group. \\\hline

\verb|CPUWeight| & CPU weight for all spawned processes combined
($1..10000$).  \texttt{systemd}'s default is 100. \\\hline

\verb|TasksMax| & Maximum number of tasks ($1..$).  \texttt{systemd}
sets no limit by default. \\\hline

\verb|MemoryMax| & Absolute limit on the combined memory usage of all
spawned processes.  Value is in bytes and may be postfixed with
\texttt{kB}, \texttt{MB}, \texttt{GB} or \texttt{TB}.
\texttt{systemd} sets no limit by default. \\\hline

\verb|IOWeight| & IO weight for all spawned processes combined
($1..10000$).  \texttt{systemd}'s default is 100. \\\hline
...
On \texttt{SIGHUP}, the error log file is reopened, all caches are
flushed and all spawned child processes are faded out (see
\ref{fadechildren}).

\subsection{Triggers}

The Debian trigger \verb|cm4all-apps-changed| reloads all spawned
applications.  It shall be invoked after updating application
packages.
...
If the filter returns status \texttt{200 OK} or \texttt{204 No
  Content}, then the previous status code is used instead.
...
and query string.  It is remembered for \verb|GET| requests to the
widget when it is focused and the XML processor is enabled.
\verb|POST| requests do not update the state because the \verb|POST|
URI may not be valid in a follow-up \verb|GET| request.  AJAX requests
on the other hand should not update the state, and they do not because
they usually do not use the XML processor, which is only useful for
...
given \verb|CHILD_TAG|. \label{fadechildren} \\
...
The Debian package \texttt{cm4all-beng-control} contains a generic
non-interactive client which can send commands to \emph{beng-proxy} or
\emph{beng-lb}.

The following command sends \verb|NOP| to an abstract socket:

\begin{verbatim*}
cm4all-beng-control --server=@bp-control nop
\end{verbatim*}

And here is a multicast on interface \texttt{eth1}:
