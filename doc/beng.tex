...
\texttt{PROCESS} & Enables the processor. \\

\hline

\texttt{CONTAINER} & Allows embedding other widgets. \\

\hline

\texttt{PREFIX\_CSS\_CLASS} & CSS class names with leading underscore
...
for a list of valid values.  Example:
...
\texttt{PROCESS\_CSS} & Enables the CSS processor. \\
...
When the option \texttt{PREFIX\_CSS\_CLASS} is enabled, CSS class
names with a leading underscore are rewritten.  The option is
available in both processors (HTML and CSS).

A single leading underscore makes the class local to the current
widget class.  It may be shared by multiple instances of the same
class.

A double leading underscore makes the class local to the current
widget instance.  Each instance may define different styles for this
class.
