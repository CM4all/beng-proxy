...
See \ref{ns} for how to configure namespaces.
...
process.  This namespace is created implicitly by the packet
\verb|PIVOT_ROOT|.  Its works like the \texttt{chroot} command; its
...
a top-level directory called \texttt{mnt}.

\verb|MOUNT_PROC| mounts a new instance of the \texttt{proc} filesystem.

\verb|MOUNT_HOME| bind-mounts the home directory (specified by
\verb|HOME|) to the given directory within the \verb|PIVOT_ROOT|.

\verb|PIVOT_ROOT| depends on user namespaces.  \verb|MOUNT_PROC|
depends on \verb|PIVOT_ROOT|, user namespaces and PID namespaces.
