...
class Translation(Protocol):
    ...
    def _handle_cron(self, partition, user, uri, param):
        log.msg("cron partition=%s user=%s uri=%s param=%s" % (repr(partition), repr(user), repr(uri), repr(param)))
    ...
    def _handle_request(self, request):
        ...
        if request.cron:
            return self._handle_cron(request.cron, request.user, request.uri, request.param)
